...
model_training_folder <- '/Users/jiangongliu/Desktop/5_WTD_Aridity/new_dataframe/model_training_elevation_Globe_Fan/'
...
common_theme <-  theme(legend.position = "bottom",
                       ...
                       plot.margin = unit(c(5, 10, 10, 5), "points")
)

x_bound <- round(max(abs(shap_long_forest$value)))
...
x_bound <- round(max(abs(shap_long_dry$value)))
...
x_bound <- round(max(abs(shap_long_crop$value)))
c <- ggplot(data = shap_long_crop) +
  ...
  labs(y = "Shapely value (impact on global SIF prediction)", x = "", color = "Feature value")

x_bound <- round(max(abs(shap_long_grass$value)))
d <- ggplot(data = shap_long_grass) +
  ...
  labs(y = "Shapley value (impact on global SIF prediction)", x = "", color = "Feature value")
...
png(filename = paste0("/Users/jiangongliu/Desktop/5_WTD_Aridity/figure/", 
                      "FigS_global_importance_wEle.png"), 
    width = 10, height = 5.5, units = "in", res = 600)
print(fig)
dev.off()
